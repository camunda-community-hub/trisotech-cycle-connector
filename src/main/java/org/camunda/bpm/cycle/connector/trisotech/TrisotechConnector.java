...
/**
 * An example connector implementation which persists BPMN files to a simple memory based map.
 *
 */
@Component
public class TrisotechConnector extends Connector {
    ...
    public void init(ConnectorConfiguration config) {
        try {
            super.init(config);
            trisotechClient = new TrisotechClient(getConfiguration().getName(), getConfiguration().getProperties().get(CONFIG_KEY_TRISOTECH_BASE_URL),
                    getConfiguration().getProperties().get(CONFIG_KEY_PROXY_URL), getConfiguration().getProperties().get(CONFIG_KEY_PROXY_USERNAME),
    ...
    public ConnectorNode createNode(String parentId, String label, ConnectorNodeType type, String message) {
        ...
        } else if (parent.getId().equals(TrisotechClient.SLASH_CHAR)) {
    ...
    public void deleteNode(ConnectorNode node, String message) {
        ...
        case FOLDER:
            ...
            if (trisoNode.getId() == TrisotechClient.SLASH_CHAR || trisoNode.getLabel() == null) {
                throw new CycleException("Cannot Delete A Root Directory");
            }

            if (!trisoNode.getId().contains(TrisotechClient.SLASH_CHAR)) {
